...

{
  ...
  while (T) {
    ...
    switch (len = getlinee(iport,buffer,20)) {
      case (AT_EOF) :
        return(ANSWER_EOF);
        break;
      case (TOO_MANY_CHARS) :
...
int getlinee (iport,buffer,buflen) FILE *iport; char *buffer; int buflen;
...

{
  int len;
  char *line;
  if (0 > (len = getlinee(fp,linebuf,maxlinelen))) {
     if (len == AT_EOF) {
...

{
  ...
  while (T) {
    ...
    if (0 > (len = getlinee(fp,linebuffer,linebuflen))) {
       fclose(fp);
       if (len == AT_EOF) {
          return(linecount);
       }
